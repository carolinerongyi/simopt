...
class DiffExampleModel(Auto_Model):
    """
    ...
    def __init__(self, fixed_factors=None):
        ...
        self.n_rngs = 1
        self.n_responses = 1
        ## need to add response names
        self.response_names = ['polynomial']
        self.factors = fixed_factors
        self.specifications = {
            "point": {
                "description": "point to evaluate",
                "datatype": tuple,
                "default": (2.0, 3.0, 4.0, 9.0)
            }
        }
    ...
    def check_simulatable_factors(self):
        return True
    ## change to inner_replicate
    def inner_replicate(self, diff_factors, rng_list, response_names):
        noise_rng = rng_list[0]
        noise = noise_rng.normalvariate()
        ## call factor_dict to get the differentiable factors
        factors = factor_dict(self,diff_factors)
        ## this needs to reference the new list of factors to be differentiable
        response = noise + 2 * factors['point_0']**1 + factors['point_1'] **2 + factors["point_2"]**3 + factors['point_3']**1.5
        responses = {'polynomial': response}
        ##return the resp_dict_to_array method
        return resp_dict_to_array(self, responses, response_names)


## I didnt change anyting past here for the autograd
"""
Summary
-------
Minimize f(x).
...
class DiffExampleProblem(Problem):
    """
    ...
    def __init__(self, name="EXAMPLE-1", fixed_factors=None, model_fixed_factors=None):
        ...
        self.model = DiffExampleModel(self.model_fixed_factors)
